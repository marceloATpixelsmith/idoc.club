...
export async function finalizeAuthenticatorReplacement(input: {
  userId: number;
  ...
  if (!Number.isSafeInteger(input.userId) || input.userId < 1 ||
    !Number.isSafeInteger(input.expectedSessionVersion) || input.expectedSessionVersion < 0 ||
    input.recoveryCodes.length === 0 || input.recoveryCodes.some((code) =>
      code.subjectId !== String(input.userId) || code.applicationId !== input.applicationId ||
      code.generationId !== input.recoveryGenerationId)) {
  ...
  return client.begin(async (tx) => {
    ...
    if (!enrollment || Number(enrollment.user_id) !== input.userId ||
      ...
      enrollment.purpose !== 'authenticator-replacement' || enrollment.consumed_at ||
      new Date(String(enrollment.expires_at)).getTime() <= nowMs) {
      return { status: 'invalid-transaction' as const };
    }
