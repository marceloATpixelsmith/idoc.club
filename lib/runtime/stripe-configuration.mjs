...
// The only hostnames a live Stripe key is required for. A Vercel Production deployment aliased to
// any other domain (a temporary redesign subdomain, say) is real member-facing traffic in Vercel's
// eyes but not in ours, so it takes a test key like Preview/Development until it's actually serving
// one of these.
const LIVE_HOSTNAMES = new Set(['idoc.club', 'www.idoc.club']);
...
function isLiveHostname(baseUrl) {
  try { return LIVE_HOSTNAMES.has(new URL(baseUrl ?? '').hostname); } catch { return false; }
}

export function stripeDeploymentMode(environment = process.env) {
  if (environment.VERCEL_ENV === 'production') return isLiveHostname(environment.BASE_URL) ? 'live' : 'test';
