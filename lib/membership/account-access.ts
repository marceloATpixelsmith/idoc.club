...
export type AccountState = 'active' | 'migrated_pending' | 'onboarding' | 'suspended' | 'unverified';
...
export function mayAccessAccountFunction(input: { accountState: AccountState; actor: Actor; entitled: boolean }, operation: AccountFunction) {
  if (input.accountState === 'suspended' || input.accountState === 'migrated_pending' || input.accountState === 'unverified') return false;
