...
/** Short-lived 6-digit codes for signup/login/password-reset/google-disconnect email
 * verification. Sent synchronously (not via the async account_delivery_outbox worker) since a
 * 30-minute-lifetime code shouldn't sit in a delivery queue. user_id is null for
 * signup_verification, where no user row exists yet until the password step completes. */
export const emailOtpCodes = idocSchema.table('email_otp_codes', {
  ...
  check('email_otp_codes_purpose_check', sql`${table.purpose} in ('signup_verification', 'login_verification', 'password_reset', 'google_disconnect_verification')`),
