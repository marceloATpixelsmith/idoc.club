...
function ActionButton({ action, children, newWindow = false, variant = 'default' }: {
  ...
  useEffect(() => {
    ...
    if (newWindow) window.open(String(state.redirectUrl), '_blank', 'noopener,noreferrer');
...
export function MembershipCard({ canManageBilling, renewalCaption, renewalDate, showRenew, statusLabel, preference, recurring, typeIcon, typeLabel }: {
