...
export default async function AdminReconciliationPage() {
  ...
  return <main className="flex-1 p-8">
    ...
    <p className="mt-2 text-sm text-gray-700">
      ...
      (suspend/reinstate, correct entitlement) from the <Link className="underline" href="/admin/members">Members page</Link>.
    </p>

    <section className="mt-6 max-w-2xl border rounded-lg p-4">
      <h2 className="font-medium text-gray-900">Last run</h2>
      {lastRun ? (
        <>
          <p className="mt-2 text-sm text-gray-700">Ran at: {lastRun.ranAt.toISOString()}</p>
          <p className="mt-1 text-sm text-gray-700">Status: {lastRun.status === 'completed' ? 'Completed' : 'Failed'}</p>
          {lastRun.status === 'failed' && <p className="mt-1 text-sm text-red-600">Error: {lastRun.errorMessage}</p>}
          <p className="mt-1 text-sm text-gray-700">Findings on that run: {lastRun.findingsCount}</p>
        </>
      ) : (
        <p className="mt-2 text-sm text-gray-500">No reconciliation run has completed yet.</p>
      )}
    </section>

    <section className="mt-8 overflow-x-auto">
      <h2 className="font-medium text-gray-900">Current findings</h2>
      <table className="mt-2 min-w-full border text-sm">
        ...
        <tbody>
          {findings.length === 0 && (
            <tr><td className="p-2 text-gray-500" colSpan={4}>No findings from the last run.</td></tr>
          )}
          {findings.map((finding) => (
            <tr key={finding.id} className="border-b align-top">
              ...
              <td className="p-2">
                {finding.profileId
                  ? <Link className="underline" href={`/admin/members?profileId=${finding.profileId}`}>View member</Link>
                  : '—'}
              </td>
