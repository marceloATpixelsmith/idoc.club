...
export default async function AdminPaymentsPage({ searchParams }: { searchParams: Promise<{ profileId?: string }> }) {
  ...
  return <main className="flex-1 p-8">
    ...
    {!selected && <p className="mt-4 text-sm text-gray-700">Search for a member on the <Link className="underline" href="/admin/members">Members page</Link> to record a payment.</p>}
    {selected && (
