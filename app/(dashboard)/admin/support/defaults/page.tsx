...
export default async function SupportDefaultsPage() {
  ...
  return <main className="space-y-6 py-8 px-5 lg:px-8"><header><h1 className="text-2xl font-semibold">Support category defaults</h1><p className="text-muted-foreground">Defaults apply only to conversations created after a change.</p></header>
    <div className="grid gap-4 md:grid-cols-3">{SUPPORT_CATEGORIES.map((category) => { const current = defaults.filter((row) => row.category === category); return <section className="rounded-lg border p-4" key={category}><h2 className="mb-3 font-semibold">{CATEGORY_LABELS[category]}</h2><SupportForm action={updateSupportCategoryDefault} submitLabel="Save default"><input name="category" type="hidden" value={category} /><label>Default administrators<select className="mt-1 block w-full border p-2" defaultValue={current.map((row) => String(row.assignment_key))} name="administratorIds" multiple required><option disabled value="">Select administrators</option>{administrators.map((admin) => <option key={String(admin.assignment_key)} value={String(admin.assignment_key)}>{String(admin.display_name)}</option>)}</select></label></SupportForm></section>; })}</div>
  </main>;
}
