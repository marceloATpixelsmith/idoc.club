...
export default async function AdminSupportThread({ params, searchParams }: { params: Promise<{ publicId: string }>; searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return <main className="space-y-6 py-8 px-5 lg:px-8"><Link className="underline" href={returnTo}>← Support inbox</Link><header><h1 className="text-2xl font-semibold">{String(conversation.subject)}</h1><p>{String(conversation.member_name)} · {String(conversation.member_email)} · {CATEGORY_LABELS[conversation.category as keyof typeof CATEGORY_LABELS]} · <strong>{STATUS_LABELS[String(conversation.status)]}</strong></p></header>
    <div className="grid gap-6 lg:grid-cols-[1fr_18rem]"><section><ol className="space-y-4">{conversation.messages.map((message, index) => <li className={`rounded-lg border p-4 ${message.author_side === 'admin' ? 'bg-muted' : ''}`} key={`${message.created_at}-${index}`}><p className="mb-2 text-sm font-semibold">{message.author_side === 'admin' ? 'Administrator' : 'Member'} · {new Date(String(message.created_at)).toLocaleString()}</p><p className="whitespace-pre-wrap break-words">{String(message.body)}</p></li>)}</ol>
      <div className="mt-6"><h2 className="mb-3 text-lg font-bold uppercase tracking-wider text-gold">Reply</h2><SupportForm action={replyToSupportAsAdmin} pendingLabel="Sending" submitLabel="Send administrator reply"><input name="idempotencyKey" type="hidden" value={randomUUID()} /><input name="publicId" type="hidden" value={publicId} /><label>Message<textarea className="mt-1 block min-h-32 w-full rounded border p-2" maxLength={10000} name="body" required /></label></SupportForm></div></section>
      <aside className="space-y-6"><section className="rounded-lg border p-4"><h2 className="mb-3 font-bold uppercase tracking-wider text-gold">Assignment</h2><SupportForm action={assignSupportConversation} submitLabel="Save assignment"><input name="publicId" type="hidden" value={publicId} /><label>Administrators<select className="mt-1 block min-h-32 w-full border p-2" defaultValue={conversation.assigned_admin_keys.map(String)} multiple name="administratorIds">{administrators.map((admin) => <option key={String(admin.assignment_key)} value={String(admin.assignment_key)}>{String(admin.display_name)}</option>)}</select><span className="mt-1 block text-xs text-muted-foreground">Use Ctrl/Command to select more than one. Clear all selections to leave unassigned.</span></label></SupportForm></section>
      <section className="rounded-lg border p-4"><h2 className="mb-3 font-bold uppercase tracking-wider text-gold">Workflow</h2><SupportForm action={changeSupportConversationStatus} submitLabel={conversation.status === 'closed' ? 'Reopen conversation' : 'Close conversation'}><input name="publicId" type="hidden" value={publicId} /><input name="operation" type="hidden" value={conversation.status === 'closed' ? 'reopen' : 'close'} /></SupportForm></section></aside></div>
  </main>;
}
