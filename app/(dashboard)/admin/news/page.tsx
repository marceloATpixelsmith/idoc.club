...
export default async function AdminNewsPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return <main className="space-y-6 px-5 py-8 lg:px-8">
    ...
    <form className="grid gap-3 rounded-lg border p-4 md:grid-cols-6" method="get">
    ...
    <ActiveFilterChips filters={[{ label: 'Search', name: 'q', value: scalar('q') }, { label: 'Status', name: 'status', value: scalar('status') }, { label: 'From', name: 'from', value: scalar('from') }, { label: 'To', name: 'to', value: scalar('to') }]} pathname="/admin/news" />
