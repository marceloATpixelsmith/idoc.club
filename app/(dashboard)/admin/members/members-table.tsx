...
export function MembersTable({ filters, initialColumnOrder, initialVisibleColumns, pageSize, rows, total }: { defaultActive: boolean; filters: Filters; initialColumnOrder?: string; initialVisibleColumns?: string[]; pageSize: number; rows: AdminMemberRow[]; total: number }) {
  ...
  const columns = useMemo<ColumnDef<AdminMemberRow>[]>(() => [
    ...
    { id: 'expires', accessorKey: 'validUntil', enableColumnFilter: true, header: header('expires'), meta: { label: 'Expiration date', variant: 'dateRange' }, cell: ({ row }) => row.original.validUntil ? new Date(`${row.original.validUntil}T00:00:00`).toLocaleDateString() : '—' },
  ...
  const { table, shallow, debounceMs, throttleMs } = useDataTable({ columns, data: rows, enableAdvancedFilter: true, getRowId: (row) => row.profileId ? `profile-${row.profileId}` : `user-${row.userId}`, initialState: { columnOrder: (searchParams.get('columnOrder') ?? initialColumnOrder)?.split(','), columnVisibility: initialVisibility, pagination: { pageIndex: filters.page - 1, pageSize }, sorting: initialSorting }, pageCount: Math.max(1, Math.ceil(total / pageSize)), queryKeys: { filters: 'filters', joinOperator: 'joinOperator', page: 'page', perPage: 'pageSize', sort: 'sort' }, shallow: false });
  ...
  useEffect(() => {
    ...
    void persistTablePreferences('memberships', { columns, columnOrder: params.get('columnOrder') ?? initialColumnOrder, filters: params.get('filters') ?? undefined, joinOperator: params.get('joinOperator') ?? undefined, pageSize: state.pagination.pageSize, q: params.get('q') ?? undefined, sort: params.get('sort') ?? undefined, status: params.get('status') ?? undefined });
    router.replace(`${pathname}?${params}`, { scroll: false });
  ...
  return <>
    ...
    <DataTable table={table} pageSizeOptions={[10, 25, 50, 100]} emptyState={<div><strong>{hasActiveView ? 'No users match this view' : 'No users exist'}</strong><span className="mt-1 block text-muted-foreground">{hasActiveView ? 'Edit or clear filters to broaden the result set.' : 'Users appear here after account creation.'}</span></div>} actionBar={<ActionBar onOpenChange={(open) => { if (!open) table.resetRowSelection(); }} open={selected > 0}><ActionBarSelection>{selected} selected</ActionBarSelection><ActionBarGroup><ActionBarItem onSelect={() => { const anchor = document.createElement('a'); anchor.href = `/api/admin/export/members?${selectedExportParams}`; anchor.download = 'selected-members.csv'; anchor.click(); }}>Export selected CSV</ActionBarItem><ActionBarItem onSelect={() => table.resetRowSelection()}>Clear selection</ActionBarItem></ActionBarGroup><ActionBarClose aria-label="Close selected-row actions"><X /></ActionBarClose></ActionBar>}>
      <DataTableAdvancedToolbar table={table} className="mt-5 rounded-xl border bg-background p-3" trailingAction={<Button asChild aria-label="Download These results" size="icon" variant="outline"><Link aria-label="Download These results" download href={`/api/admin/export/members?${exportParams}`} title="Download These results"><Download aria-hidden="true" /></Link></Button>}>
