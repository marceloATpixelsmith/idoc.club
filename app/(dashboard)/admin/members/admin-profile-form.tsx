...
export function AdminProfileForm({ member, profileId }: { member: Member; profileId: number }) {
  ...
  return <form action={action} className="max-w-2xl space-y-4">
    ...
    {classification !== 'veterinarian' ? <fieldset className="space-y-4 border p-4"><legend>Official information</legend><Select initial={official?.nationalFederationCountryCode ?? ''} label="National Federation" name="nationalFederationCountryCode" required values={ISO_COUNTRY_CODES} /><Select initial={official?.idocRegion ?? ''} label="IDOC Region" name="idocRegion" required values={IDOC_REGIONS} /><label className="block">FEI ID<input className="block w-full border p-2" defaultValue={official?.feiId ?? ''} name="feiId" required /></label>{classification === 'judge' || classification === 'judge_steward' ? <><CheckboxGroup initial={judge?.officialStatuses ?? []} label="Official Status as Judge" name="judgeStatus" values={JUDGE_STATUSES} /><Select initial={judge?.isTechnicalDelegate ? 'yes' : 'no'} label="Technical Delegate" name="isTechnicalDelegate" required values={['yes', 'no']} /></> : null}{classification === 'steward' || classification === 'judge_steward' ? <CheckboxGroup initial={steward?.officialStatuses ?? []} label="Official Status as Steward" name="stewardStatus" values={STEWARD_STATUSES} /> : null}</fieldset> : null}
