...
export default async function AdminMembersPage({ searchParams }: { searchParams: Promise<MemberFilters & { profileId?: string }> }) {
  ...
  return <main className="flex-1 py-8 px-5 lg:px-8">
    ...
    <div className="mt-4 flex items-center justify-between text-sm"><span>{listing.total} matching members</span><Link className="underline" download href={`/api/admin/export/members?${new URLSearchParams(Object.entries(params).filter(([,value]) => value !== undefined).map(([key,value]) => [key,String(value)]))}`}>Export filtered CSV</Link></div>
