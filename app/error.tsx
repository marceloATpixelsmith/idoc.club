...
function isAuthorizationError(error: Error): boolean {
  return error.name === 'AuthorizationError' || error.message === 'You are not authorized to perform this action.';
}
  ...
  return (
    <main className="mx-auto flex min-h-[70dvh] max-w-lg items-center px-5 py-16 text-center">
      ...
      <div className="mx-auto mb-7 flex size-16 items-center justify-center rounded-full bg-primary/10 text-primary">
        <span className="text-2xl font-semibold">{isAuthorizationError(error) ? '404' : '!'}</span>
      </div>
      <h1 className="text-2xl font-medium text-foreground mb-2">{isAuthorizationError(error) ? 'This page isn’t available' : 'Something went wrong'}</h1>
      <p className="text-muted-foreground mb-6">
        {isAuthorizationError(error) ? 'You do not have access to this area. If you followed a link, it may be intended for administrators only.' : 'We hit an unexpected problem. Please try again, or return to the home page.'}
