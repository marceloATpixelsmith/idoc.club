...
- **Known unresolved risk, not yet mitigated: the async delivery outbox has no environment partition.** `claimAccountDelivery()` (`lib/notifications/account-delivery.ts`) leases the oldest eligible row from the single shared `idoc.account_delivery_outbox` table with no predicate distinguishing which deployment queued it. Distinct `BREVO_API_KEY`/`BREVO_WEBHOOK_KEY` values (above) do not fix this: if `/api/cron/account-delivery` is ever invoked against `staging.idoc.club` (e.g. for a staging Cron UAT check per §12.1 below) while a real production password-reset/migration-activation row is queued, staging can claim and send it — through staging's Brevo credentials, with a link built from staging's `BASE_URL` — to a real member. The reverse (production claiming a staging test row) is less harmful but still wrong. This needs partitioning the queued work by deployment (an origin/environment column plus a claim predicate, or fully separate queues) before it's safe to invoke the staging Cron endpoint against real data; until that lands, treat any staging Cron UAT of this endpoint as unsafe to run for real and get explicit operator sign-off first.
...
5. **`git log origin/main..origin/staging` showing commits is normal**, not drift — it's the backlog of changes verified on staging and awaiting deliberate promotion. Checking the reverse direction needs one adjustment: GitHub's merge-commit strategy means every legitimate `staging` → `main` promotion PR creates a merge commit that lives only on `main` (its second parent is `staging`'s tip, but the merge commit itself is never reachable from `staging`), so `git log origin/staging..origin/main` is *expected* to show that one merge commit after every valid promotion — that alone is not an anomaly. Use `git log --no-merges origin/staging..origin/main` (or `git diff origin/staging origin/main` for actual content) instead: a clean promotion leaves either of those empty. A non-merge commit showing up there, or a non-empty content diff, means something reached production without passing through staging; treat that as the real anomaly to investigate and, if it wasn't an authorized emergency hotfix, back-merge it into `staging` right away.
