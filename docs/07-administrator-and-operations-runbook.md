...
Preferences follow the signed-in administrator across browsers and devices. URL parameters override a saved view for the request. **Reset to default** removes the table's preference row; it never changes records or another administrator's settings. If saving fails, the URL remains usable and the administrator may retry after refreshing.
