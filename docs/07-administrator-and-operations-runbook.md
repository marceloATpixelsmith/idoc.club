...
This section is the authoritative production-auth configuration inventory. The application is one Vercel-hosted Next.js deployment; its Cron route runs in that deployment and no separate authentication worker is deployed elsewhere. Put server-only values in **Vercel Project Settings → Environment Variables**, scoped separately to Production and to the protected Preview/Staging deployment used for UAT. Preview/Staging must use its own non-production provider credentials and cryptographic secrets. **Two deliberate exceptions:** `staging.idoc.club` shares the single production Render PostgreSQL instance (`POSTGRES_URL`) with production rather than paying for a second instance (see "Branch, environment, and deployment workflow" above), and the Google OAuth client is shared with production (see §15.3) — only `GOOGLE_OAUTH_REDIRECT_URI` differs per environment. All instances within one environment must receive the same compatible values. Never put real values in `.env.example`, Git, documentation, issues, pull requests, chat, screenshots, build output, or runtime logs.
