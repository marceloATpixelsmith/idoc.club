...
The Memberships roster now uses a unified toolbar for full-name/email search, Active/Expired/Archived views, expiration range, federation, address country, IDOC region, membership type, sorting, columns, page size, selection, filtered CSV export, and pagination. Active is visibly marked as the default only for an administrator with no saved Memberships preference and no URL state. Filter chips are independently removable; clearing filters persists the cleared/default view. Empty results distinguish an empty system from a filtered view with no matches. Row menus link to the established audited edit, payment history/manual payment, expiration extension, membership-type, email, and seminar-history workflows.
