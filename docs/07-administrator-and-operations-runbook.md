...
5. **`git log origin/main..origin/staging` showing commits is normal**, not drift — it's the backlog of changes verified on staging and awaiting deliberate promotion, and it grows again the moment any new ordinary work lands on `staging`, including right after a clean promotion. Checking for an actual bypass needs a check that stays correct as that backlog grows, not a log or content diff against `staging`'s current (constantly moving) tip — `git log --no-merges origin/staging..origin/main` and `git diff origin/staging origin/main` both go non-empty again the instant the next ordinary commit lands on `staging`, which is expected, not a bypass, and neither command can tell the two cases apart. Use `git merge-base --is-ancestor origin/main origin/staging` instead (exit code `0` means clean): this asks only "is everything currently on `main` also reachable from `staging`," which stays true no matter how far ahead `staging` subsequently gets. A nonzero exit means `main` has a commit `staging` never saw; treat that as the real anomaly to investigate and, if it wasn't an authorized emergency hotfix, back-merge it into `staging` right away.
