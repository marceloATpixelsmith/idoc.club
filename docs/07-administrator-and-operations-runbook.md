...
The onboarding "Keep me updated" consent checkbox (docs/02 section 1.3) additionally uses `MAILCHIMP_MARKETING_API_KEY` and `MAILCHIMP_AUDIENCE_ID` — a separate product/credential from `MAILCHIMP_TRANSACTIONAL_API_KEY` above (Mailchimp Marketing audience subscriptions vs. Mailchimp Transactional/Mandrill one-off sends). Unlike the privileged boundary list above, these two are **not** required for the runtime to start: they gracefully degrade. If either is missing, malformed, or the Mailchimp Marketing API is unreachable, the subscription attempt is skipped or fails silently (categorical logging only, matching the OTP-delivery-failure pattern), and onboarding always completes regardless — a member's own account-standing, payment, security, and renewal messages never depend on this opt-in succeeding.
