...
The initial slice added reusable trusted-server MFA primitives and migration `0020_durable_mfa_persistence` added `PostgresMfaStore`. The first live integration now enforces authenticator-app enrollment and routine login TOTP for Administrator and Super Admin grants after both password and Google primary authentication. Ordinary members retain the existing login behavior. Password-reset MFA, general step-up, factor management, and member remembered devices remain separate follow-up work.
