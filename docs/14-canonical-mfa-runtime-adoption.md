...
This slice adds reusable trusted-server MFA primitives only. It does **not** yet change IDOC's login, password-reset, Google OIDC, session-establishment, or account-settings behavior, and it does not add database persistence or user-facing MFA routes. Those integrations remain separate follow-up work.
...
The framework-neutral adapter interface is intentionally preserved. IDOC must still provide a database-backed implementation that atomically enforces enrollment consumption, TOTP replay prevention, challenge transaction purpose/subject/application binding, attempt limits, recovery-code single use, remembered-device validity and revocation, and factor lifecycle transitions.

IDOC application roles currently use `administrator` and `super_admin` authorization grants. The runtime's canonical role vocabulary remains `admin` and `super-admin`; the persistence/policy integration layer must perform an explicit trusted-server mapping rather than accepting a client-provided role string.

## Required follow-up before MFA is enabled

A later integration slice must add durable MFA persistence and wire the runtime into the authoritative authentication state machine. At minimum that work must include:

1. forward-only IDOC migrations for factors, enrollment/challenge transactions, recovery codes, and remembered devices;
2. a trusted IDOC `MfaStore` implementation with atomic database operations and bounded attempts;
