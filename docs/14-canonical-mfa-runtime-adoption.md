...
9. **Implemented:** `/dashboard/security` is the canonical member-facing management surface for password and Google sign-in, owned active sessions, ordinary-member remembered login devices, privileged authenticator/recovery entry, and account deletion. Current-session identity comes from the server-owned canonical session; other-session and device revocation preserve ownership and do not expose credentials. Privileged replacement reuses the recovery-authorized state machine. Standalone recovery-code regeneration is implemented on the same security surface for an authenticated privileged user with an active TOTP factor and fresh `generate-recovery-codes` step-up; it atomically invalidates the old code set without replacing the authenticator and returns the new plaintext set only once;
