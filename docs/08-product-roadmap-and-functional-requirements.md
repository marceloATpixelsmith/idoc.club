...
The Memberships and Support rosters use the official Dice UI/Tablecn composition (`DataTable`, one advanced filter list, sort list, view options, pagination, `useDataTable`, and `ActionBar`). Memberships keeps editable filter rows inside one Filter panel, supports multiple filters and clear-all behavior, selectable rows, column controls, sortable operational columns, rows-per-page selection, bounded server pagination, filtered CSV export, responsive overflow, and distinct loading/empty/error states. Its country, federation, and region filters reuse the profile's choices; option filters exclude empty operators. The server applies multiple filter operators and ordered sorts. Filter, Sort, and View lists start alphabetically, Sort offers visible columns, and View reorders columns left to right and persists that order per administrator. Rows show country names, uppercase membership type and status with icons, and consistent date formats; the filtered export control is an icon at the toolbar's right edge. Active remains the default, while deliberate filters reach expired/archived, no-active-membership, privileged, onboarding/migration-pending, and conventionally named test accounts. It exposes only the existing privacy-bounded member row and links to established audited single-member workflows.

News, Seminars, and Content Pages now share the Dice UI administrator resource table. Each offers server-backed search, status and date/audience filters where applicable, advanced title/status filters, sortable columns, a 10/25/50/100 page-size selector, column visibility, row selection, an export of selected visible-page rows, and direct links to existing edit/preview/registration workflows. Selection is transient, and the exported CSV contains only records already fetched for the authorized administrator. Table preferences remain owner-scoped and validated; URL state takes precedence. These tables do not add direct bulk publishing or deletion. Notification history and reconciliation findings use a read-only Dice UI table with local search, filtering, sorting, pagination, columns, and audited selected-row CSV export; those two report views do not persist preferences or add database identifiers. Payment/refund forms, revenue aggregates, and detail-page history remain separate operational views.
