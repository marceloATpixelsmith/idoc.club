...
After the admin layout authorizes access, its error boundaries present the page path, UTC time, server digest when available, and a server-generated log reference. Administrators may expand and copy browser-side technical details for troubleshooting. Client-reported error text is never sent to server logs; production server exception details remain in protected runtime logs.
...
An initial seminar management and registration slice is implemented: Tablecn-style administrator authoring with server-side search/status/date filtering, stable sorting, pagination, and column controls at `/admin/seminars` and public/member registration at `/seminars`. A signed-out visitor sees a flat, unauthenticated list of available seminars with no tabs. A paid/grace member instead sees two top-level tabs: **My Seminars** (the default landing view, showing the member's own current and past registrations via a secondary Upcoming & current/Past sub-nav) and **Available Seminars** (the database-backed catalog of upcoming, published seminars the member has not yet registered for, with the registration action). The implementation uses the same three canonical Organization Settings payment methods rather than a seminar-specific payment configuration. See [Release 5](#8-release-5---seminars) below for full scope and what remains deferred, and [07 Administrator and Operations Runbook](07-administrator-and-operations-runbook.md#seminar-operations) for operations. Migration `0041` owns this durable history.
...
The Stripe acceptance follow-up adds scenario-aware automated traceability and executable, opt-in
Stripe test-mode seminar Checkout/refund and browser-security coverage. It does not move the release
gate: provider execution requires the disposable test environment in docs/09, and Dashboard,
Stripe-originated HTTPS webhook, Vercel, backup/restore, alerting, and operator sign-off evidence
remain manual production-readiness blockers.
...
The administrator layout supplies the `nuqs` Next.js App Router adapter to its child routes so URL-driven table controls can render. This fixes the browser error observed on `/admin/members` when the adapter was missing.

The shared administrator table foundation now persists validated per-user state for Memberships, Support, News, Seminars, and Content Pages in migration `0045`. Durable state is restricted to table-specific search/filter values, allow-listed sorting and visible columns, rows per page, and applicable view state. The URL represents the current view and overrides a saved row; absent URL state, a valid saved row precedes defaults. Memberships defaults to Active only when neither exists. Every write is authenticated, administrator-authorized, CSRF-protected, owner-derived on the server, and upserted by the unique user/table key. The `DELETE /api/admin/table-preferences/[table]` endpoint still removes only the actor's saved table row, but no toolbar exposes it as a control; each table's toolbar instead offers a single **Reset** button that clears the table's active filters, matching the tablecn.com reference toolbar. Transient selection and destructive workflow state are excluded.

The Memberships and Support rosters use the official Dice UI/Tablecn composition (`DataTable`, toolbar, sort list, view options, pagination, `useDataTable`, and `ActionBar`). Memberships shows its filters as simple multi-select checkbox facet dropdowns and a calendar date-range picker directly next to search -- not an advanced filter-builder panel -- supports clear-all behavior via Reset, selectable rows, column controls, sortable operational columns, rows-per-page selection, bounded server pagination, filtered CSV export, responsive overflow, and distinct loading/empty/error states. Its country, federation, and region filters reuse the profile's choices. The server applies the selected filter values and ordered sorts. Sort and View lists start alphabetically, Sort offers visible columns and supports custom multi-column ordering from its own popover, and View reorders columns by dragging a handle and persists that order per administrator. Rows show country names, uppercase membership type and status with icons, and consistent date formats; the filtered export control is an icon at the toolbar's right edge. Active remains the default, while deliberate filters reach expired/archived, no-active-membership, privileged, onboarding/migration-pending, and conventionally named test accounts. It exposes only the existing privacy-bounded member row and links to established audited single-member workflows.

News, Seminars, and Content Pages now share the Dice UI administrator resource table. Each offers server-backed search next to simple multi-select checkbox status (and audience, for Content Pages) facet filters and a calendar date-range filter where applicable, a custom multi-column sort list alongside per-column header sorting, a 10/25/50/100 page-size selector, drag-to-reorder column visibility, row selection, an export of selected visible-page rows, and direct links to existing edit/preview/registration workflows. Selection is transient, and the exported CSV contains only records already fetched for the authorized administrator. Table preferences remain owner-scoped and validated; URL state takes precedence. These tables do not add direct bulk publishing or deletion. Notification history and reconciliation findings use a read-only Dice UI table with local search, filtering, sorting, pagination, columns, and audited selected-row CSV export; those two report views do not persist preferences or add database identifiers. Payment/refund forms, revenue aggregates, and detail-page history remain separate operational views.

Administrator route error boundaries now share one retry and reference display. An expected authorization denial is classified by the administrator layouts before their navigation-count query; database and runtime failures retain their error evidence.

The selected-row menu supports an audited, server-authorized CSV export of selected visible-page members and copying selected support-conversation links. Production bulk archive/pause/revoke remains gated: it must reuse suspension and Super-Admin incident response, preserve support and seminar history, snapshot future-registration identity before detaching a member, cancel every Stripe subscription, remove Mailchimp audience membership, report mixed provider outcomes, and be idempotently retryable. No partial implementation is authorized. This replaces the older generic table note with an explicit release gate; it does not change current billing, retention, entitlement, or authorization semantics.
