...
- Billing Settings shows the membership amount, the Renewal Date (the paid-through date -- the day the membership needs to be renewed by), and the current renewal preference as a single Automatic/Manual choice with any pending change and its effective date. Stripe Customer Portal may manage payment methods and invoices, but IDOC owns the membership-specific automatic-renewal preference and transition workflow.
