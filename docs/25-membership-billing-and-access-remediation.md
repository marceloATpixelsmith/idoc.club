...
| Payment-only gate on the member dashboard | `app/(dashboard)/dashboard/layout.tsx`, `dashboard/page.tsx`, `dashboard/dashboard-tabs.tsx` | A never-paid or post-grace-expired member visiting `/dashboard` is redirected straight to `/pricing` (no intermediate "Pay for membership" screen); every other dashboard sub-page (Profile, Security -- which now also carries Activity as a table at its bottom -- and Seminars) redirects back to `/dashboard` as a real per-request check, which in turn redirects on to `/pricing`. Administrators/Super Admins are never gated by this, whether or not they even have a member profile at all, and see the full tab bar regardless of their own entitlement. |
