...
| Payment-only gate on the member dashboard | `app/(dashboard)/dashboard/layout.tsx`, `dashboard/page.tsx`, `dashboard/membership/page.tsx`, `dashboard/dashboard-tabs.tsx` | A never-paid or post-grace-expired member visiting `/dashboard` is routed to `/dashboard/membership`, which renders the membership payment controls directly with no standalone pricing page or intermediate paywall screen. Other entitlement-gated dashboard sub-pages redirect back through `/dashboard`, which resolves to the same My Membership payment surface. Administrators/Super Admins are never gated by this, whether or not they even have a member profile at all, and see the full tab bar regardless of their own entitlement. |
...
|---|---|---|
| Member-facing payment surface | `app/(dashboard)/dashboard/membership/page.tsx` | Implemented: one Membership Payment panel with one automatic-renewal control lives inside My Membership; the standalone `/pricing` route has been removed. |
