...
## Security-completeness evidence sweep

The item-8 implementation adds a deduplicated, recipient-snapshotted extension of the existing authentication-security outbox for password changes/resets, verified-email changes (old and new address), authenticator enrollment/replacement, recovery-code use, privileged role transitions, and explicit other-session mass revocation. Matching append-only audit actions contain only actor/subject, factor or method category, role/reason, and timestamps. Password/factor/role version changes continue to invalidate persisted sessions, ordinary trust, pending MFA continuations, and step-up authority; notification provider availability is not part of mutation success. Existing negative PostgreSQL suites remain the behavioral authority for replay, binding, ownership, expiry, eligibility, single-use and final-Super-Admin concurrency boundaries.
