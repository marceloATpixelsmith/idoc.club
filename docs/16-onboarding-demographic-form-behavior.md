...
After the password or Google signup step creates the authenticated account, the account opens My Membership inside the dashboard (`/dashboard`, which hosts the onboarding wizard directly for an account still in the `onboarding` state). Until profile onboarding and payment are complete, the other dashboard navigation is hidden and direct requests to those pages return the account to `/dashboard`. The header continues to provide sign-out.

Signup and ordinary login land differently once an account is fully set up: a signup always continues to `/dashboard` (onboarding, or My Membership once active). An ordinary login (password or Google) for an account that has already finished onboarding now lands on the site's public homepage (`/`) instead of the dashboard -- an account still mid-onboarding is always sent to `/dashboard` regardless of how it authenticated, so the wizard is never skipped.
...
The header's "Contact" nav item is hidden for a signed-in, `entitled` account -- both an ordinary entitled member (whose My IDOC dropdown carries its own "Support/Contact" entry, `/dashboard/support`) and a privileged administrator/super_admin (who uses `/admin/support` instead, and whose My IDOC dropdown filters Support out entirely). A signed-in account that is not yet `entitled` -- still onboarding, or unpaid/expired -- keeps the Contact link, since its My IDOC is only a plain `/dashboard/membership` link to the onboarding/payment surface, with no Support destination to fall back on.

Each dashboard nav item's URL matches its label: My Membership lives at `/dashboard/membership` (the bare `/dashboard` entry point hosts onboarding and otherwise redirects there), My Profile at `/dashboard/profile`, My Security at `/dashboard/security`, and Support/Contact at `/dashboard/support`. `lib/navigation/dashboard-nav.ts` is the single source of these entries, consumed by both the dashboard's own left-hand sidebar and the header's "My IDOC" dropdown.
