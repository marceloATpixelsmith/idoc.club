...
the sixth (AUTH-OPERATIONS-004's WebAuthn ceremony simulation gap is closed; a narrower residual gap around
proving the exact concurrent-race `'replay'` status end to end through the full ceremony, rather than as
two separately-proven halves, remains and is disclosed in its row) — see their rows in document 22 for the
full evidence. Current status is **144 verified, 0 implemented-but-unverified, 4 partial, 0 missing, and 7
...
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| AUTH-OPERATIONS-004 | See the authoritative canonical machine contract. | partial | WebAuthn registration/removal/login are now proven through a real simulated ceremony; the specific `'replay'` status (as opposed to the library's own earlier stale-snapshot rejection of a simple resubmission) is reached only via a genuine concurrent-request database race, which is not forced deterministically in the new adversarial test. | See docs/22 current implementation column. | Real production path named in docs/22. | See docs/22. | Existing evidence is catalogued in docs/22. | The race that actually produces the `'replay'` status through the full real ceremony remains unproven end to end, though each of its two halves (the store's non-increasing-counter rejection; the action layer's `'replay'`-to-`mfa_replay_detected` wiring) is proven separately. | TEST | low | conditional | Force the genuine concurrent-request race deterministically (e.g. two real ceremonies both reading a stale credential snapshot before either's store write commits) if judged worth the added test complexity and potential flakiness, rather than relying on the two separately-proven halves. | Real production caller with genuine dependency, database, cryptographic, or browser behavior as applicable. | Corrective authentication/security remediation | Existing production infrastructure. | no | no |
