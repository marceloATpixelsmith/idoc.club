...
evidence), each with real behavioral evidence, current status is **139 verified, 0
implemented-but-unverified, 9 partial, 0 missing, and 7 not-applicable**. Consequently, the table below
has exactly **9 applicable non-verified controls**.
...
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| AUTH-CRYPTO-003 | See the authoritative canonical machine contract. | partial | Current evidence does not capture every canonical output boundary during real MFA flows. | See docs/22 current implementation column. | Real production path named in docs/22. | See docs/22. | Existing evidence is catalogued in docs/22. | Behavioral coverage is incomplete for the stated property. | TEST | high | conditional | Capture logs, audit, notifications, responses, browser storage, and exports with sentinel secrets through production flows. | Real production caller with genuine dependency, database, cryptographic, or browser behavior as applicable. | Corrective authentication/security remediation | Existing production infrastructure. | yes | yes |
| AUTH-PRIVACY-001 | See the authoritative canonical machine contract. | partial | Analytics absence evidence scans only a narrow source sample and does not behaviorally prove every export authorization boundary. | See docs/22 current implementation column. | Real production path named in docs/22. | See docs/22. | Existing evidence is catalogued in docs/22. | Behavioral coverage is incomplete for the stated property. | TEST | high | conditional | Scan the complete production source and build output and exercise real bounded, owner/admin, and cross-user exports. | Real production caller with genuine dependency, database, cryptographic, or browser behavior as applicable. | Corrective authentication/security remediation | Existing production infrastructure. | yes | yes |
| AUTH-DEPENDENCY-001 | See the authoritative canonical machine contract. | partial | PostgreSQL and Stripe posture is supported materially by source inspection rather than forced failure through every production caller. | See docs/22 current implementation column. | Real production path named in docs/22. | See docs/22. | Existing evidence is catalogued in docs/22. | Behavioral coverage is incomplete for the stated property. | TEST | high | conditional | Behaviorally force each registered dependency failure through its real production path, including retry, replay, and dead-letter cases. | Real production caller with genuine dependency, database, cryptographic, or browser behavior as applicable. | Corrective authentication/security remediation | Existing production infrastructure. | yes | yes |
| AUTH-OPERATIONS-004 | See the authoritative canonical machine contract. | partial | WebAuthn replay and passkey registration/removal event wiring remains source-inspection evidence. | See docs/22 current implementation column. | Real production path named in docs/22. | See docs/22. | Existing evidence is catalogued in docs/22. | Behavioral coverage is incomplete for the stated property. | TEST | high | conditional | Use generated WebAuthn key material through real actions and prove replay denial plus secret-free durable events and delivery. | Real production caller with genuine dependency, database, cryptographic, or browser behavior as applicable. | Corrective authentication/security remediation | Existing production infrastructure. | yes | yes |
| AUTH-OPERATIONS-006 | See the authoritative canonical machine contract. | partial | Rate-limit alert delivery is awaited in the authentication critical path and lacks a durable concurrency-safe retry handoff. | See docs/22 current implementation column. | Real production path named in docs/22. | See docs/22. | Existing evidence is catalogued in docs/22. | Behavioral coverage is incomplete for the stated property. | TEST | high | conditional | Persist a deduplicated alert transactionally, deliver through a leased retry worker, and prove timeout/error/concurrency behavior. | Real production caller with genuine dependency, database, cryptographic, or browser behavior as applicable. | Corrective authentication/security remediation | Existing production infrastructure. | yes | yes |
| AUTH-OPERATIONS-007 | See the authoritative canonical machine contract. | partial | Incident revocation omits pending authority and atomic/idempotent operations-alert evidence and lacks full adversarial action tests. | See docs/22 current implementation column. | Real production path named in docs/22. | See docs/22. | Existing evidence is catalogued in docs/22. | Behavioral coverage is incomplete for the stated property. | TEST | high | conditional | Invalidate all standing and pending authority with stable incident idempotency, durable owner/operations alerts, and stage-failure tests. | Real production caller with genuine dependency, database, cryptographic, or browser behavior as applicable. | Corrective authentication/security remediation | Existing production infrastructure. | yes | yes |
