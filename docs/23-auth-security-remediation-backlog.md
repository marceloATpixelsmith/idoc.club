...
evidence). A subsequent Codex audit found three of those "closed" controls — AUTH-DEPENDENCY-001,
AUTH-OPERATIONS-006, and AUTH-OPERATIONS-007 — had evidence gaps (source-inspection-only Postgres/Stripe
proof; an alert awaited on the authentication-adjacent hot path with no durable retry; and incomplete
idempotency, pending-authority, and operations-alert coverage, respectively) and reopened them to
`partial`; a further pull request closed all three with real behavioral evidence (a genuinely unreachable
Postgres connection and a genuinely failing Stripe API call driven through their real production callers;
a durable leased-retry outbox moving alert delivery off the authentication hot path; and a DB-enforced
idempotency guarantee plus a durable operations alert and adversarial pending-authority/partial-failure
tests for incident response) — see their rows in document 22 for the full evidence. Current status is
**142 verified, 0 implemented-but-unverified, 6 partial, 0 missing, and 7 not-applicable**. Consequently,
the table below has exactly **6 applicable non-verified controls**.
