...
evidence). A subsequent Codex audit found four of those "closed" controls — AUTH-DEPENDENCY-001,
AUTH-OPERATIONS-006, AUTH-OPERATIONS-007, and AUTH-PRIVACY-001 — had evidence gaps (source-inspection-only
Postgres/Stripe proof; an alert awaited on the authentication-adjacent hot path with no durable retry;
incomplete idempotency, pending-authority, and operations-alert coverage; and an analytics-absence scan
covering only one sampled file plus incomplete export-route authorization coverage, respectively) and
reopened them to `partial`; a further pull request closed all four with real behavioral evidence (a
genuinely unreachable Postgres connection and a genuinely failing Stripe API call driven through their
real production callers; a durable leased-retry outbox moving alert delivery off the authentication hot
path; a DB-enforced idempotency guarantee plus a durable operations alert and adversarial
pending-authority/partial-failure tests for incident response; and an analytics scan broadened to the
complete production source tree plus the real build output, alongside full owner/admin/cross-user
route-level export-authorization coverage) — see their rows in document 22 for the full evidence. Current
status is **143 verified, 0 implemented-but-unverified, 5 partial, 0 missing, and 7 not-applicable**.
Consequently, the table below has exactly **5 applicable non-verified controls**.
