...
evidence). A subsequent Codex audit found six of those "closed" controls — AUTH-DEPENDENCY-001,
AUTH-OPERATIONS-006, AUTH-OPERATIONS-007, AUTH-PRIVACY-001, AUTH-CRYPTO-003, and AUTH-OPERATIONS-004 — had
evidence gaps (source-inspection-only Postgres/Stripe proof; an alert awaited on the
authentication-adjacent hot path with no durable retry; incomplete idempotency, pending-authority, and
operations-alert coverage; an analytics-absence scan covering only one sampled file plus incomplete
export-route authorization coverage; passkey_registered/passkey_removed secret-absence confirmed only by
code reading, not a scan; and WebAuthn replay/registration/removal event wiring proven only at the source
level, never through a full simulated ceremony, respectively) and reopened them to `partial`; a further
pull request closed the first five with real behavioral evidence (a genuinely unreachable Postgres
connection and a genuinely failing Stripe API call driven through their real production callers; a durable
leased-retry outbox moving alert delivery off the authentication hot path; a DB-enforced idempotency
guarantee plus a durable operations alert and adversarial pending-authority/partial-failure tests for
incident response; an analytics scan broadened to the complete production source tree plus the real build
output, alongside full owner/admin/cross-user route-level export-authorization coverage; and a real, minimal
WebAuthn authenticator simulator -- a genuine ES256 keypair, real CBOR attestation, real DER-encoded signed
assertions -- driven through the real production registration/removal/login Server Actions) and narrowed
the sixth with a deterministic two-ceremony barrier at the real production credential-read boundary,
proving the exact concurrent-race `'replay'` status end to end without timing-dependent sleeps — see the
verified AUTH-OPERATIONS-004 row in document 22 for the full evidence. Current status is **145 verified,
0 implemented-but-unverified, 3 partial, 0 missing, and 7 not-applicable**. Consequently, the table below
...
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| AUTH-SECRET-004 | Validate production secret strength and separation for every security purpose. | partial | Pepper is explicitly optional and absent, so that half is not applicable. OAuth replacement, rollback, retirement, and secret-free audit are implemented. A permanent `/admin/security` operation now removes the local-tooling barrier: the live Server Action requires CSRF, server-loaded Super Admin authority, and fresh purpose-bound MFA; the production data operation selects the active version from server configuration and makes concurrent/repeated submissions idempotent. The only remaining gap is external evidence: use the deployed button for the already-completed rotation and confirm its production audit row. | `lib/auth/google-oidc-reference.ts`; `lib/auth/google-oidc-secret-audit.ts` (`recordActiveGoogleOauthSecretRotation`); `app/(dashboard)/admin/security/*` | Real deployed Super Admin action and production audit row; repository tests cannot substitute for the completed provider rotation. | `idoc.audit_log` | `tests/google-oauth-secret-rotation.test.ts`; `tests/google-oauth-secret-audit.integration.ts`; `tests/authorization-boundary-inventory.test.ts`; real Google sign-in already completed by the operator | Code path is complete and adversarially covered; classification remains truthful until the production button is used and the resulting evidence is confirmed. | UAT | low | operational | Deploy this change, sign in as Super Admin, open `/admin/security`, select **Record completed rotation**, complete fresh MFA, and confirm the page reports active version `2026-09-03` as recorded. | Preserve a safe screenshot/timestamp and audit-row identifier only; never capture a secret value. | Slice 5 | Production application and `idoc.audit_log`. | no | no |
...
4. **CSRF/request integrity — AUTH-CSRF-003. COMPLETE — verified.** CODE+TEST. A signed double-submit-
   ...
   **Known gap found in a later session (unresolved):** the "removed CSRF cookie" half of the two-test
   ...
   itself remain proven; only that one specific behavioral test (now `test.fixme` and split out into
   `tests/security-e2e/csrf-cookie-removal.spec.ts`) needs redesigning.
