...
The intentional public partial-prerender shells are `/`, `/_not-found`, `/pricing`, `/privacy`, `/request-activation`, and `/terms`. `/sign-up`, `/sign-in`, and `/recover-password` each read their own pending-flow step from a signed cookie on every request and are therefore dynamic, not static shells. Authenticated/token-consuming routes and every route handler are dynamic. Dashboard activity/general/security have data-free client shells; member data is supplied only by dynamic server streams or authenticated runtime routes. Route classification is asserted rather than changed to mask import-time effects.
