...
The intentional public partial-prerender shells are `/`, `/_not-found`, `/privacy`, `/request-activation`, and `/terms`. `/dashboard/membership` is the authenticated membership/payment surface and remains dynamic. `/sign-up`, `/sign-in`, and `/recover-password` each read their own pending-flow step from a signed cookie on every request and are therefore dynamic, not static shells. Authenticated/token-consuming routes and every route handler are dynamic. Dashboard activity/general/security have data-free client shells; member data is supplied only by dynamic server streams or authenticated runtime routes. Route classification is asserted rather than changed to mask import-time effects.
...
Payment lifecycle acceptance must cover distinct seminar prices/titles, server-owned metadata and customer ownership, duplicate checkout/webhook/refund attempts, cancellation before and after payment, failed/direct/partial refunds, disputes and chargebacks, cross-member and administrator boundaries, and proof that seminar activity never mutates membership entitlement or history. Refund cases must conform to [10 Refund Policy](10-refund-policy.md). Deploy migration `0049` before enabling administrator refunds.
