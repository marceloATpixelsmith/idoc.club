...
| **Environment**   | **Branch** | **Domain** | **Purpose**                        | **Data**                                                                                                                                                                                                             |
|-------------------|------------|------------|-------------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Local/development | any        | n/a        | Feature development.               | Synthetic data only.                                                                                                                                                                                                 |
| Vercel Preview    | any PR     | per-PR preview URL | Pull-request testing. **Not currently active:** the Vercel project is not configured to build a preview deployment for any branch other than `staging` — this row describes the intended design for ordinary per-PR previews if that's ever turned on, not something running today. | **Intended design:** dedicated non-production Render PostgreSQL database or isolated non-production schema, with an isolated Drizzle migration ledger and Stripe test mode; preview deployments must not migrate the production database. **Dormant gap (`docs/07` "Branch, environment, and deployment workflow"):** `POSTGRES_URL` and most other secrets are not currently scoped to the `staging` branch specifically, so if per-PR previews were ever enabled, they would receive the real production values rather than this intended isolation. Not exploitable today because no such previews are built; worth closing before ever enabling them. |
| Staging           | `staging`  | `staging.idoc.club` | **The pre-merge gate.** Every change lands here first for migration rehearsal, UAT, and the Claude Code live-auth audit before it is promoted to Production (see `docs/07` "Branch, environment, and deployment workflow"). | **Deliberately near-identical to Production**, not sanitized/isolated — shares the Render PostgreSQL instance, `idoc` schema/migration ledger, and most of the secret inventory verbatim (avoids a second Render Postgres cost and keeps staging verification predictive of Production behavior; see `docs/07`). `BASE_URL`/`GOOGLE_OAUTH_REDIRECT_URI`, the Turnstile testing keys, the Stripe credentials (must diverge at the go-live domain cutover — see `docs/07` "Stripe payment production readiness"), and the Brevo API/webhook keys are the exceptions that stay distinct. Stripe test mode today. |
| Production        | `main`     | `redesign.idoc.club` today; `idoc.club`/`www.idoc.club` at go-live cutover | Live idoc.club membership service. Receives only reviewed promotions from Staging, never a feature branch directly. | Shared production Render PostgreSQL database using the dedicated idoc schema and idoc.\_\_drizzle_migrations ledger, plus the existing production Stripe account. Connections require TLS/SSL.                       |
...
## Administrator data-table preferences

Migration `0045` adds `administrator_table_preferences`, one row per authenticated administrator or Super Admin and stable table identifier (`memberships`, `support`, `news`, `seminars`, or `content_pages`). A unique `(user_id, table_identifier)` index makes updates an upsert and prevents one person's layout from becoming another person's default. `preferences` is JSONB, but the application accepts only each table's allow-listed search, filter, sort, column, page-size, and view keys. Unknown, malformed, obsolete, and unauthorized values are discarded rather than evaluated. Selection, menu, loading, confirmation, and bulk-operation state are never durable.

The URL is the shareable current-view authority. If a URL contains table-state parameters, validated URL state wins. With no URL state, the server loads and validates the current user's saved row; table defaults apply only when no valid row exists. Preference writes resolve the owner from the authenticated server session, reauthorize administrator access, validate CSRF evidence, and never accept a browser-supplied user ID. Reset deletes only that actor's row for that table.


## Membership perks data model

`idoc.membership_perks` is the ordered, server-owned list of perk labels displayed on each membership-tier box on the public membership page and in the dashboard payment box. Super Admins replace the complete list transactionally; labels are trimmed and capped at 200 characters, no more than 50 rows are accepted, display order is explicit, and every changed replacement is audit logged. Concurrent replacements are serialized on a stable transaction-level lock so the audit record reflects the committed prior list.

Migration `0049` completes durable payment lifecycle evidence. Seminar registrations retain the Checkout Session, Payment Intent, expected EUR amount, checkout lifecycle, payment timestamps, disputes, and chargebacks. `payment_refunds` links each idempotent Stripe refund to exactly one original seminar registration or membership payment and preserves amount, status, reason, administrator, provider evidence, failure, and timestamps. Reconciliation findings cover unmatched refunds, partial-policy conflicts, disputes, chargebacks, and seminar payment conflicts. The governing policy is [10 Refund Policy](10-refund-policy.md).
