...
| Vercel Preview    | any PR     | per-PR preview URL | Pull-request testing.              | Dedicated non-production Render PostgreSQL database or isolated non-production schema, with an isolated Drizzle migration ledger and Stripe test mode. Preview deployments must not migrate the production database. |
| Staging           | `staging`  | `staging.idoc.club` | **The pre-merge gate.** Every change lands here first for migration rehearsal, UAT, and the Claude Code live-auth audit before it is promoted to Production (see `docs/07` "Branch, environment, and deployment workflow"). | **Deliberately shares production's Render PostgreSQL instance and `idoc` schema/migration ledger** (avoids a second Render Postgres cost — see `docs/07`); Stripe test mode. Data created here is real database data, not sanitized/isolated. |
