...
| Staging           | `staging`  | `staging.idoc.club` | Migration rehearsal, UAT, and the Claude Code live-auth audit. | **Deliberately shares production's Render PostgreSQL instance and `idoc` schema/migration ledger** (avoids a second Render Postgres cost — see `docs/07` "Branch, environment, and deployment workflow"); Stripe test mode. Data created here is real database data, not sanitized/isolated. |
| Production        | `main`     | `redesign.idoc.club` today; `idoc.club`/`www.idoc.club` at go-live cutover | Live idoc.club membership service. | Shared production Render PostgreSQL database using the dedicated idoc schema and idoc.\_\_drizzle_migrations ledger, plus the existing production Stripe account. Connections require TLS/SSL.                       |
