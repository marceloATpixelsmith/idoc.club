...
| Staging           | `staging`  | `staging.idoc.club` | **The pre-merge gate.** Every change lands here first for migration rehearsal, UAT, and the Claude Code live-auth audit before it is promoted to Production (see `docs/07` "Branch, environment, and deployment workflow"). | **Deliberately near-identical to Production**, not sanitized/isolated — shares the Render PostgreSQL instance, `idoc` schema/migration ledger, and nearly the entire secret inventory verbatim (avoids a second Render Postgres cost and keeps staging verification predictive of Production behavior; see `docs/07`). Only `BASE_URL`/`GOOGLE_OAUTH_REDIRECT_URI` and the Turnstile testing keys differ. Stripe test mode. |
