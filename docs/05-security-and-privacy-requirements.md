...
| Security headers              | `middleware.ts` generates a fresh CSPRNG nonce for every request, forwards the nonce-bearing CSP to Next.js so framework-rendered scripts use it, and returns the same policy on every page and API response. Production `script-src` permits only self, the per-response nonce, and Cloudflare Turnstile; it contains neither `unsafe-inline` nor `unsafe-eval`. Development alone adds `unsafe-eval` because Turbopack requires it for hot updates. Images are limited to self and data URLs. Turnstile remains explicitly allowed for scripts, frames, and connections. `style-src 'unsafe-inline'` is the sole production inline exception because Next.js/Tailwind emit framework and component style attributes without a complete nonce hook. HSTS, frame, MIME, referrer, and route-specific Permissions-Policy headers remain enforced. Dynamic per-request rendering is deliberate: PPR/client segment caching is disabled so a response nonce cannot be reused from cache. See AUTH-HEADER-001/003. |
...
- **Public members map and paid member directory** (`/about/members-directory`, the member-only tab at `/about/members-directory?tab=directory`) never expose a member's name, email, exact address, exact coordinate, or any raw/sequential database identifier to the public map, and never expose email, exact address, or a raw/sequential database identifier to the paid directory. The public map (`lib/directory/aggregate.ts`) is unauthenticated and returns only a country code and a member count, aggregated server-side and never computed or filtered client-side; a country renders only once it has at least `DIRECTORY_MIN_AGGREGATION_THRESHOLD` (currently 5) currently-entitled members, and a below-threshold country is omitted entirely rather than merged into a visible "other" total, since a merged bucket next to a displayed grand total would let a reader back-calculate its size — for the same reason no grand/world total is ever displayed alongside the thresholded per-country counts. The paid directory (`lib/directory/member-directory.ts`) is restricted to the same paid/grace/expired/suspended/unpaid access rule as every other member surface (`requireAccountAccess('member')`, re-asserted independent of the calling page), scoped to currently-entitled members, and returns only first/last name, country, professional role and level, federation, and IDOC region — the same fields the directory's original placeholder page documented as its eventual public content. Enumeration and bulk-harvesting are bounded structurally, not just by convention: page size is fixed server-side (never client-supplied), total reachable depth is capped, search input length is capped and escaped before use in a `LIKE` pattern, sort order is stable (name, then a non-exposed internal id as a pure tiebreaker) so pagination never skips or repeats a row, and sustained searching is rate-limited per account and per origin using the existing `lib/security/rate-limit.ts` facility.
...
- News/Blog article management (`/admin/news`) requires Administrator or Super Admin authority, re-asserted server-side in every `lib/news/articles.ts` export independent of the calling UI, plus the canonical CSRF evidence check on every mutating Server Action (`app/(dashboard)/admin/news/actions.ts`). Rich-text article content is sanitized server-side against an explicit tag/attribute allowlist (`lib/news/sanitize.ts`) before every insert or update and again immediately before rendering. `dangerouslySetInnerHTML` has two call sites, both on that same allowlist-sanitized value, never on raw input: the public/member-facing shared article-rendering component (`components/news/article-view.tsx`), and the administrator-only rich-text editor (`components/news/article-content-editor.tsx`), which re-sanitizes previously-stored content immediately before loading it back into the editor for further editing -- the sanitizer (and the equivalent `sanitizeBankInstructions` in `lib/organization/format.ts`, used the same way by the Organization Settings bank-instructions editor) is idempotent against its own already-sanitized output, so repeated edit/save cycles never accumulate corruption. Public reads (the `/news` listing and `/news/[slug]` pages) query only rows where `status='published' AND publication_date<=now()`, so a draft, scheduled, or archived article is never publicly reachable by any URL, guessed or otherwise. Scheduled-publication transitions are evaluated exclusively against PostgreSQL's `now()` (UTC) by the `/api/cron/news-scheduled-publish` Cron route, gated by the same `CRON_SECRET` bearer header as every other Cron route. Deleting an article permanently is allowed only from `draft` or `archived` status; a published or scheduled article must be archived first. Every publication-affecting action is audited.
...
## Signup membership intent and Turnstile presentation

A signup `membership` intent is accepted only when it equals `judge`, `steward`, `judge_steward`, or `veterinarian`. Email signup carries the validated value in the signed pending-signup cookie; Google signup carries it in the server-owned OAuth return path. It controls only initial card presentation and never bypasses the canonical server-side profile schema or the member's final choice.

Auth email-entry forms use Cloudflare Turnstile's managed `interaction-only` appearance. The widget stays hidden while loading and when verification passes silently, and becomes visible only when Cloudflare requires user interaction. Application-detected load/render failures remain visible and actionable. Server-side token, hostname, and action verification remains mandatory and fail-closed.
...
| Google identity link/unlink | `auth.google_identity.linked` / `.unlinked` | Yes | Fresh password plus privileged step-up; no implicit identity-by-email. A Google-only account (no known password) instead proves identity with a one-time email code before creating a password and unlinking in the same action | identity-ownership PostgreSQL tests |
...
## Automatic privileged step-up continuation

Every authenticated Administrator or Super Admin action requiring fresh TOTP automatically resumes and completes the original action after successful TOTP. No second submission is required. Protected forms use an inline, accessible authenticator challenge and retain the original `FormData` only in transient browser memory. Pending and fresh-step-up cookies contain only purpose-, user-, session-, session-version-, role-, factor-, transaction-, and action-bound evidence; they never contain passwords, OAuth secrets, recovery codes, MFA secrets, or mutation input. After TOTP acceptance, the browser invokes the same production Server Action with the original form data. That action repeats CSRF validation, account-state and authoritative-role authorization, session binding, validation, and the atomic single-use authority claim before mutation. Closing, expiring, navigating away from, or failing the challenge discards the in-memory input and requires an explicit restart; verification replay cannot repeat a mutation.
