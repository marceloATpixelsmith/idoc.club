...
Every issued OTP code, regardless of which of the three purposes issued it, keeps the properties already implemented in `lib/auth/email-otp.ts`: single-use, 30-minute expiry, CSPRNG-generated, stored only as a SHA-256 digest, a 30-second resend cooldown, a 5-attempt-per-code verification lockout, and a `purpose` binding (`signup_verification`, `login_verification`, `password_reset`) that a code issued for one purpose can never satisfy for another. Verification attempts are additionally subject to the same IP/email rate limiting as issuance, not just the per-code attempt lockout, so repeatedly requesting fresh codes cannot be used to grind through more guesses than the per-code lockout alone would allow.

## Device-trust ("remember me") for login verification

A returning member on a device that has never completed login verification (or whose trust has expired/been revoked) is emailed a 6-digit code (purpose `login_verification`), exactly like the existing legacy-activation OTP but for ordinary returning members rather than only `migrated_pending` accounts. That code-entry screen offers a "Remember me for 2 weeks" checkbox. Checking it issues a second, separate signed cookie (distinct from the `session` cookie and from every flow's pending-step cookie) that, for 2 weeks, lets that specific device skip login verification for that specific account; leaving it unchecked skips issuing that cookie entirely, so the next login on that device verifies again. This is unrelated to the account-creation email-verification step and unrelated to the anonymous password-reset/migration-activation link mechanism. Administrators and Super Admins never receive this option — every privileged login re-verifies the second factor (TOTP, below), with no persistent device trust.

## Administrator and Super Admin multi-factor authentication (TOTP)

Administrator and Super Admin accounts authenticate with an authenticator-app TOTP code as their second factor, both at login (after password, in place of the emailed OTP ordinary members use) and at password reset (in place of the emailed OTP), never with a device-trust cookie. A privileged account with no TOTP secret registered yet is walked through registration (secret generation + authenticator-app enrollment, e.g. via a scannable QR code) before it can complete login. No such mechanism currently exists in this codebase — no TOTP secret storage, no registration flow, no verification. Building it requires: a per-user encrypted TOTP secret column (matching the existing `ACCOUNT_DELIVERY_ENCRYPTION_KEYS`-style versioned-key-ring pattern already used for account-delivery payloads, not a new one-off encryption scheme), a registration flow reachable only by an already-authenticated administrator/super-admin whose account has no secret yet, and a verification step that never falls back to the ordinary emailed-OTP path for a privileged account.
