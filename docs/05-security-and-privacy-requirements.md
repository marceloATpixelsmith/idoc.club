...
| Security headers              | `next.config.ts` deploys, on every route including `/api/*`: a CSP (`default-src 'self'`, `frame-ancestors 'none'`, an explicit `challenges.cloudflare.com` allowance for Turnstile in `script-src`/`connect-src`/`frame-src`), `X-Frame-Options: DENY`, `X-Content-Type-Options: nosniff`, `Referrer-Policy: strict-origin-when-cross-origin`, and `Permissions-Policy: camera=(), microphone=(), geolocation=()`. No application-level `Strict-Transport-Security` header is configured anywhere in this repository; any HSTS guarantee in production depends entirely on the hosting platform's own defaults and is not verified by this repository's tests. See [Authentication security control inventory](21-authentication-security-control-inventory.md) AUTH-HEADER-001/003. |
...
| Passkey registered / removed | `auth.mfa.passkey.registered` / `.removed` | Yes | Fresh step-up required for both; registration additionally requires an active TOTP factor; TOTP requirement is never disabled by either | WebAuthn store/runtime tests |
