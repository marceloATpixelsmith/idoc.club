...
`codex/review-complete` is informational and is not required by either branch ruleset; the repository owner removed it from both required-check lists on 25 September 2026. It records only that a Codex review was requested and does not certify that a review completed. Codex review is advisory; `Fast PR checks` remain required by the rulesets, while the risk-classified full workflows below are mandatory under the agent merge procedure. The workflow requests Codex review when a revision is opened or updated, then exits without polling. Later review events update the informational status only when they refer to the current PR revision. The audited quota-waiver workflow is no longer required for CI progress.
