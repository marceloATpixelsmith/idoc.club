...
For Stripe-backed paid/grace members, the dashboard's own Payment Method subpage (`/dashboard/payment-method`) can generate a Stripe Customer Portal session server-side, letting the member update the card on file. The application must verify that the authenticated user owns the Stripe Customer ID before creating the portal session. This Portal Configuration is scoped to `payment_method_update` only -- never `invoice_history` or `subscription_cancel` -- so a member is never able to cancel or view invoices through Stripe's own UI; IDOC's dashboard is the sole place recurrence, cancellation, and payment history are tracked (docs/02 §5.1). The session also sets `flow_data` to `type: 'payment_method_update'` with an `after_completion` redirect, so it deep-links straight into Stripe's add/update card form instead of the portal's own home page, and returns the member to `/dashboard/payment-method` the instant the card is saved rather than leaving them on a Stripe-hosted confirmation screen. The session's `return_url` brings the member back to that same subpage either way (a plain same-tab navigation; there is no separate membership page left open elsewhere to protect with a new window). IDOC Billing Settings, not a generic Stripe plan selector, controls whether the membership renews automatically.
