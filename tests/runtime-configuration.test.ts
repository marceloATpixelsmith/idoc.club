...
import {
  ...
  stripeKeyForServer,
  stripeMembershipProductIdForServer,
...
test('malformed URLs, undersized secrets, and malformed provider settings fail categorically', () => {
  ...
  assert.throws(() => stripeMembershipProductIdForServer({ STRIPE_MEMBERSHIP_PRODUCT_ID: 'not-a-product' }), /STRIPE_MEMBERSHIP_PRODUCT_ID/);
  assert.equal(stripeMembershipProductIdForServer({ STRIPE_MEMBERSHIP_PRODUCT_ID: 'prod_fixture123' }), 'prod_fixture123');
