...
import { MEMBER_SESSION_ABSOLUTE_SECONDS, MEMBER_SESSION_IDLE_SECONDS, SESSION_IDLE_SECONDS } from '../lib/auth/session-tokens.ts';
...
test('the security page session list excludes an ordinary-member session only after the 7-day idle window', async () => {
  await resetIdoc();
  const user = await createUser();
  const now = new Date();

  const stillIdleWithinWindow = randomUUID();
  const activeAuthenticatedAt = new Date(now.getTime() - 24 * 60 * 60 * 1000);
  await registerSession({ sessionId: stillIdleWithinWindow, userId: user.id, sessionVersion: 0,
    authenticatedAt: activeAuthenticatedAt, lastActivityAt: activeAuthenticatedAt,
    absoluteExpiresAt: new Date(activeAuthenticatedAt.getTime() + MEMBER_SESSION_ABSOLUTE_SECONDS * 1000) });

  const idleStale = randomUUID();
  const staleLastActivity = new Date(now.getTime() - (MEMBER_SESSION_IDLE_SECONDS + 60) * 1000);
  const staleAuthenticatedAt = new Date(now.getTime() - 8 * 24 * 60 * 60 * 1000);
  await registerSession({ sessionId: idleStale, userId: user.id, sessionVersion: 0,
    authenticatedAt: staleAuthenticatedAt, lastActivityAt: staleLastActivity,
    // Still inside the member session's fixed 14-day absolute deadline; only idle staleness excludes it.
    absoluteExpiresAt: new Date(staleAuthenticatedAt.getTime() + MEMBER_SESSION_ABSOLUTE_SECONDS * 1000) });

  const active = await listActiveSessions(user.id, 0);
  const ids = active.map((session) => session.sessionId);
  assert.ok(ids.includes(stillIdleWithinWindow), 'a one-day-idle ordinary-member session must still be listed');
  assert.ok(!ids.includes(idleStale), 'a member session idle past MEMBER_SESSION_IDLE_SECONDS must not be listed');
