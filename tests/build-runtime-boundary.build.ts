...
test('real production build prerenders without DNS, TCP, HTTP, database, Stripe, or email access', { timeout: 180_000 }, () => {
  ...
  try {
    ...
    for (const route of ['/', '/_not-found', '/recover-password', '/request-activation', '/sign-in', '/sign-up']) assert.match(output, new RegExp(`◐ ${route.replace('/', '\\/')}(?:\\s|$)`));
    for (const route of ['/activate', '/api/cron/account-delivery', '/api/stripe/checkout', '/api/stripe/webhook', '/api/team', '/api/user', '/dashboard', '/dashboard/profile', '/onboarding', '/pricing', '/reset-password', '/verify-email']) assert.match(output, new RegExp(`ƒ ${route.replaceAll('/', '\\/')}(?:\\s|$)`));
...
test('production modules contain server-only boundaries and no fake credential fallbacks', () => {
  const privileged = ['lib/db/drizzle.ts', 'lib/auth/session.ts', 'lib/payments/stripe.ts', 'lib/security/encrypted-payload.ts', 'lib/notifications/account-delivery.ts', 'lib/notifications/mailchimp-transactional.ts', 'lib/notifications/profile-change-delivery.ts', 'lib/runtime/configuration.ts', 'app/api/cron/account-delivery/route.ts'];
