...
test('real production build prerenders without DNS, TCP, HTTP, database, Stripe, or email access', { timeout: 180_000 }, () => {
  ...
  try {
    ...
    for (const route of ['/', '/_not-found', '/activate', '/admin', '/admin/exports', '/admin/members', '/admin/notifications', '/admin/payments', '/admin/reconciliation', '/dashboard', '/dashboard/activity', '/dashboard/general', '/dashboard/payments', '/dashboard/profile', '/dashboard/security', '/mfa', '/onboarding', '/pricing', '/privacy', '/recover-password', '/request-activation', '/reset-password', '/sign-in', '/sign-up', '/terms', '/verify-email', '/.well-known/security.txt', '/api/address/autocomplete', '/api/admin/export/audit-log', '/api/admin/export/members', '/api/admin/export/notifications', '/api/admin/export/payments', '/api/auth/google/callback', '/api/auth/google/link/start', '/api/auth/google/link/status', '/api/auth/google/start', '/api/client-error', '/api/cron/account-delivery', '/api/cron/data-retention-purge', '/api/cron/reconciliation-scan', '/api/cron/renewal-notice-delivery', '/api/cron/renewal-notice-scan', '/api/health', '/api/mailchimp/webhook', '/api/stripe/checkout', '/api/stripe/webhook', '/api/team', '/api/user']) assert.match(output, new RegExp(`ƒ ${route.replaceAll('/', '\\/')}(?:\\s|$)`));
...
test('local and CI Release 1 gates are fail-fast and contain every required boundary', () => {
  ...
  assert.equal(scripts['check:release1'], 'node scripts/validate-auth-docs.mjs && node scripts/validate-release-checklist.mjs && node scripts/check-whitespace.mjs && pnpm typecheck && pnpm test:ci && pnpm test:integration-db && pnpm test:build-boundary && pnpm build');
