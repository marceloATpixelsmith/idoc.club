...
// portable JSON, logs and errors." GOOGLE_OAUTH_CLIENT_SECRET_VERSIONS is included here alongside
// IDOC's own self-generated secrets so the same real-build scan below (not source inspection)
// proves it too never reaches any browser-visible build output. GOOGLE_OAUTH_CLIENT_ID and
...
test('real production build prerenders without DNS, TCP, HTTP, database, Stripe, or email access', { timeout: 180_000 }, () => {
  ...
  try {
    ...
    for (const route of ['/', '/_not-found', '/activate', '/admin', '/admin/exports', '/admin/members', '/admin/notifications', '/admin/payments', '/admin/reconciliation', '/dashboard', '/dashboard/profile', '/dashboard/security', '/seminars', '/mfa', '/onboarding', '/dashboard/membership', '/privacy', '/recover-password', '/request-activation', '/reset-password', '/sign-in', '/sign-up', '/terms', '/verify-email', '/.well-known/security.txt', '/api/address/autocomplete', '/api/admin/export/audit-log', '/api/admin/export/members', '/api/admin/export/notifications', '/api/admin/export/payments', '/api/auth/google/callback', '/api/auth/google/link/start', '/api/auth/google/link/status', '/api/auth/google/start', '/api/client-error', '/api/cron/account-delivery', '/api/cron/data-retention-purge', '/api/cron/reconciliation-scan', '/api/cron/renewal-notice-delivery', '/api/cron/renewal-notice-scan', '/api/health', '/api/brevo/webhook', '/api/stripe/checkout', '/api/stripe/webhook', '/api/team', '/api/user']) assert.match(output, new RegExp(`ƒ ${route.replaceAll('/', '\\/')}(?:\\s|$)`));
