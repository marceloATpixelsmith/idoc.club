...
test('real production build prerenders without DNS, TCP, HTTP, database, Stripe, or email access', { timeout: 180_000 }, () => {
  ...
  try {
    ...
    for (const route of ['/activate', '/admin', '/admin/exports', '/admin/members', '/admin/notifications', '/admin/payments', '/api/admin/export/audit-log', '/api/admin/export/members', '/api/admin/export/notifications', '/api/admin/export/payments', '/api/cron/account-delivery', '/api/stripe/checkout', '/api/stripe/webhook', '/api/team', '/api/user', '/dashboard', '/dashboard/payments', '/dashboard/profile', '/onboarding', '/reset-password', '/verify-email']) assert.match(output, new RegExp(`ƒ ${route.replaceAll('/', '\\/')}(?:\\s|$)`));
...
test('production modules contain server-only boundaries and no fake credential fallbacks', () => {
  const privileged = ['lib/db/drizzle.ts', 'lib/auth/session.ts', 'lib/payments/stripe.ts', 'lib/payments/manual-payments.ts', 'lib/security/encrypted-payload.ts', 'lib/notifications/account-delivery.ts', 'lib/notifications/mailchimp-transactional.ts', 'lib/notifications/profile-change-delivery.ts', 'lib/notifications/renewal-notices.ts', 'lib/membership/status-actions.ts', 'lib/membership/role-grants.ts', 'lib/membership/exports.ts', 'lib/runtime/configuration.ts', 'app/api/cron/account-delivery/route.ts', 'app/api/cron/renewal-notice-scan/route.ts', 'app/api/cron/renewal-notice-delivery/route.ts', 'app/api/admin/export/members/route.ts', 'app/api/admin/export/payments/route.ts', 'app/api/admin/export/audit-log/route.ts', 'app/api/admin/export/notifications/route.ts'];
