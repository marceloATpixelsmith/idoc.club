...
test('real production build prerenders without DNS, TCP, HTTP, database, Stripe, or email access', { timeout: 180_000 }, () => {
  ...
  try {
    ...
    for (const route of ['/', '/_not-found', '/pricing', '/privacy', '/recover-password', '/request-activation', '/terms']) assert.match(output, new RegExp(`◐ ${route.replace('/', '\\/')}(?:\\s|$)`));
    for (const route of ['/activate', '/admin', '/admin/exports', '/admin/members', '/admin/notifications', '/admin/payments', '/admin/reconciliation', '/api/admin/export/audit-log', '/api/admin/export/members', '/api/admin/export/notifications', '/api/admin/export/payments', '/api/client-error', '/api/cron/account-delivery', '/api/cron/reconciliation-scan', '/api/cron/renewal-notice-delivery', '/api/cron/renewal-notice-scan', '/api/stripe/checkout', '/api/stripe/webhook', '/api/team', '/api/user', '/dashboard', '/dashboard/payments', '/dashboard/profile', '/onboarding', '/reset-password', '/sign-in', '/sign-up', '/verify-email']) assert.match(output, new RegExp(`ƒ ${route.replaceAll('/', '\\/')}(?:\\s|$)`));
