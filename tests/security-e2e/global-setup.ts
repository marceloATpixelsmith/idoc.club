...
export default async function globalSetup() {
  ...
  await migrate(drizzle(sql), { migrationsFolder: new URL('../../lib/db/migrations', import.meta.url).pathname, migrationsSchema: 'idoc', migrationsTable: '__drizzle_migrations' });
  ...
  for (const name of STATES) {
    ...
    await writeFile(`.security-e2e/${name}.json`, JSON.stringify({ cookies: [{ name: 'idoc-session', value: token, domain: '127.0.0.1', path: '/', expires: Math.floor(expires.getTime() / 1000), httpOnly: true, secure: false, sameSite: 'Lax' }], origins: [] }));
  }
  await sql.end();
}
