...
test('forward recovery repairs migrations skipped after an out-of-order production ledger', async () => {
  ...
  try {
    ...
    const [{ count }] = await sql<{ count: number }[]>`select count(*)::int as count from idoc.__drizzle_migrations`;
    assert.equal(count, 48, 'the ledger records applied timestamps; skipped historical files are repaired by migration 0051');
  } finally {
...
test('generated migration metadata agrees with the migrated schema', async () => {
  ...
  assert.equal(journal.entries[24].tag, '0024_account_state_check_constraint');
  assert.ok(journal.entries[24].when > journal.entries[23].when, 'the account_state CHECK constraint must follow migration 0023');
  assert.equal(journal.entries[25].tag, '0025_new_sign_in_notification_kind');
  assert.ok(journal.entries[25].when > journal.entries[24].when, 'the new_sign_in notification kind must follow migration 0024');
  assert.equal(journal.entries[26].tag, '0026_webauthn_credentials');
  assert.ok(journal.entries[26].when > journal.entries[25].when, 'the WebAuthn credentials migration must follow migration 0025');
  assert.equal(journal.entries[27].tag, '0027_passkey_notification_kinds');
  assert.ok(journal.entries[27].when > journal.entries[26].when, 'the passkey notification kinds must follow migration 0026');
  assert.equal(journal.entries[28].tag, '0028_account_suspension_notification_kinds');
  assert.equal(journal.entries[29].tag, '0029_recovery_codes_regenerated_notification');
  assert.ok(journal.entries[29].when > journal.entries[28].when, 'recovery-code regeneration migration must follow migration 0028');
  assert.ok(journal.entries[28].when > journal.entries[27].when, 'the account suspension notification kinds must follow migration 0027');
  assert.equal(journal.entries[30].tag, '0030_email_display_form');
  assert.ok(journal.entries[30].when > journal.entries[29].when, 'the email display-form migration must follow migration 0029');
  assert.equal(journal.entries[31].tag, '0031_mfa_replay_notification_kind');
  ...
  assert.equal(journal.entries[35].tag, '0035_remove_webauthn_passkeys');
  assert.ok(journal.entries[35].when > journal.entries[34].when, 'the passkey/WebAuthn removal migration must follow migration 0034');
  assert.equal(journal.entries[36].tag, '0036_session_device_label');
  ...
  assert.equal(journal.entries[40].tag, '0040_news_articles');
  ...
  for (const tableName of ['webauthn_credentials', 'webauthn_ceremony_challenges']) {
    assert.equal((await sql`select 1 from information_schema.tables where table_schema='idoc' and table_name=${tableName}`).length, 0, `idoc.${tableName} must have been dropped by migration 0035`);
  }
  assert.equal((await sql`select 1 from information_schema.columns where table_schema='idoc' and table_name='mfa_factors' and column_name='encrypted_secret' and is_nullable='NO'`).length, 1, 'mfa_factors.encrypted_secret must be NOT NULL after WebAuthn removal');
  assert.equal((await sql`select 1 from information_schema.columns where table_schema='idoc' and table_name='mfa_factors' and column_name='encryption_key_id' and is_nullable='NO'`).length, 1, 'mfa_factors.encryption_key_id must be NOT NULL after WebAuthn removal');
  assert.equal((await sql`select 1 from pg_constraint where conname='mfa_factors_totp_secret_check'`).length, 0, 'mfa_factors_totp_secret_check must have been dropped by migration 0035');
  assert.equal((await sql`select 1 from information_schema.columns where table_schema='idoc' and table_name='users' and column_name='name'`).length, 0, 'users.name must have been dropped by migration 0037');
  const [typeCheck] = await sql<{ definition: string }[]>`select pg_get_constraintdef(oid,true) as definition from pg_constraint where conname='mfa_factors_type_check'`;
  assert.deepEqual((typeCheck.definition.match(/'[^']*'/g) ?? []).sort(), ["'totp'"], 'mfa_factors_type_check must permit only totp after WebAuthn removal');
...
test('final migrated catalog exactly agrees with the authoritative Drizzle snapshot', async () => {
  ...
  // Migration 0035 removed passkey/WebAuthn support: this frozen 0030 snapshot predates that removal,
  // so its two WebAuthn tables and its mfa_factors entry (which allowed a 'webauthn' factor_type and
  // left the TOTP secret columns nullable to accommodate it) no longer describe the live schema. This
  ...
  expectedMfaFactors.columns.encrypted_secret.notNull = true;
  expectedMfaFactors.columns.encryption_key_id.notNull = true;
  delete expectedMfaFactors.checkConstraints.mfa_factors_totp_secret_check;
  expectedMfaFactors.checkConstraints.mfa_factors_type_check.value = '"idoc"."mfa_factors"."factor_type" in (\'totp\')';

  // Migration 0053 added the nullable users.password_set_at column after this frozen snapshot was
  ...
  for (const [qualifiedName, expectedTable] of Object.entries<any>(expectedSchema)) {
    ...
    const expectedConstraints: any[] = [
      ...Object.values<any>(expectedTable.foreignKeys).map((value) => ({ name: tableName === 'membership_checkout_sessions' && value.columnsFrom[0] === 'profile_id' ? 'membership_checkout_sessions_profile_id_fkey' : value.name.slice(0, 63), type: 'f', columns: value.columnsFrom, target_schema: value.schemaTo, target_table: value.tableTo, target_columns: value.columnsTo, update_action: actionCode(value.onUpdate), delete_action: actionCode(value.onDelete) })),
      ...Object.values<any>(expectedTable.uniqueConstraints).map((value) => ({ name: value.name.slice(0, 63), type: 'u', columns: value.columns })),
      ...Object.values<any>(expectedTable.checkConstraints).map((value) => ({ name: value.name, type: 'c' })),
      ...Object.values<any>(expectedTable.compositePrimaryKeys).map((value) => ({ name: value.name.slice(0, 63), type: 'p', columns: value.columns })),
