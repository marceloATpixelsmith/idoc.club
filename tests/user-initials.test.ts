...
test('a single-word name uses only that initial', () => {
  assert.equal(userInitials('Cher', null, 'cher@example.test'), 'C');
});

test('a missing name falls back to the first letter of the email, in caps', () => {
  assert.equal(userInitials(null, null, 'zeta@example.test'), 'Z');
  assert.equal(userInitials(undefined, undefined, 'zeta@example.test'), 'Z');
});
