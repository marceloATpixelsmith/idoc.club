...
const actionFiles: Record<string, Record<string, 'session-boundary' | 'pre-authentication' | 'authenticated-continuation' | 'delegates-to-data-access'>> = {
  ...
  'app/(dashboard)/dashboard/membership-actions.ts': { cancelMembershipAction: 'delegates-to-data-access' },
...
test('delegates-to-data-access actions call an ownership-enforcing membership data-access function', () => {
  const expected: Record<string, Array<{ from: string; functionName: string }>> = {
    ...
    'app/(dashboard)/dashboard/membership-actions.ts': [{ from: '@/lib/membership/data-access', functionName: 'cancelOwnMembership' }],
...
test('the Brevo webhook Route Handler verifies the shared webhook key before parsing or acting on any event', () => {
  const source = readFileSync(path.join(root, 'app/api/brevo/webhook/route.ts'), 'utf8');
  const verify = source.indexOf('verifyBrevoWebhookKey('); const parse = source.indexOf('parseBrevoEvent(');
